...
#'
#' \code{BCHydroSub2018TW} returns the ground-motion prediction with it sigma of Abrahamson et al.(2016) GMPE.
#'
...
#'
#' @examples
#' BCHydroSub2018TW(6, 20, 0, 0, 760, 1, 10, 20)
#' BCHydroSub2018TW(7, 10, 0, 0, 760, 1, 10, 20)
#'
#' @export
BCHydroSub2018TW <- function(Mag, Rrup, Prd, ftype=0, Vs30, forearc=1, depth, Rhypo) {
  if (Prd != 0 & (Prd < 0.02 | Prd > 10)) {
    stop("Period out of range! \n\n")
  }
  #subroutine S05_BCHydroSub_Taiwan ( mag, fType, rRup, vs30, lnSa, sigma1, specT, period1, iflag, forearc, depth, disthypo,
  #                                   deltac1 )
  retvals <- .Fortran("S05_BCHydroSub_Taiwan", mag=as.single(Mag), fType=as.single(ftype), rRup=as.single(Rrup),
